...
- `自动随机动作`：空闲时自动播放动作。
- `视线跟随`：桌宠视线跟随鼠标位置。
- `鼠标追随`：默认关闭；开启后桌宠会平滑地追着鼠标移动，并保留一点距离避免遮挡光标。开启时拖拽释放可把桌宠扔远，前两次会在停下后大咩，动作结束后慢慢飞回鼠标旁边；第三次会伤心并自动关闭追随。
- `躲避鼠标`：默认关闭；开启后桌宠会在鼠标靠近时自然绕开，耳翼和尾翼会跟随逃离方向、速度和受惊程度摆动。该模式会和 `鼠标追随` 互斥。
